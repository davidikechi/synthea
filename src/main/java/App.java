...
public class App {
   ...
   *
   * @param args Existing command line arguments.
   * @param moduleName Module name to include in generation.
   * @return Updated argument list containing the requested module filter.
   */
  public static String[] withModule(String[] args, String moduleName) {
    ...
    for (int i = 0; i < argList.size(); i++) {
      if (argList.get(i).equalsIgnoreCase("-m")) {
        if (i + 1 >= argList.size()) {
          throw new IllegalArgumentException("-m requires a module list value");
        }

        String existingModules = argList.get(i + 1);
        String[] modules = existingModules.split(File.pathSeparator);
        ...
        argList.set(i + 1, existingModules + File.pathSeparator + moduleName);
        return argList.toArray(new String[0]);
      }
    }
